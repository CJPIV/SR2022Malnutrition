...
TRAINING = list(None, InfoGain, mRMR, JMI, MVLoR, ChiSq)
...
RF_Accuracy = data.frame(Stunting = double(),
                     StuntingMale = double(),
                     StuntingFemale = double(),
                     StuntingUrban = double(),
                     StuntingNotUrban = double(),
...
for (set in 1:19){
  for (i in 1:10){
    for (j in 1:10){
        training = data[[set]][[i]][[j]]
        if (set == 3 | set > 7){ #SMOTEd data is one layer deeper
          training = training$data
        }
      RF = train(Stunting~., data = training, method = "rf", trControl = )
    }
  }
}
