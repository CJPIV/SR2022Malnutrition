...
for (i in 1:3){ #3 response sets (Stunting, Thinness, Wasting)
  for (j in 1:5){ #5 feature selection (FS) sets
    ...
    for (k in 1:10){ #10 test sets total
      test = TEST[[i]][[k]] #Prepare test; 1 test set for 10 train/valid sets (seen below)
      #test = as.matrix(test)
...
# Я тоже хочу сделать что-то о тех, что мы расговаривали. Наш модель ещё не работает достаточно хорошо за публикации, и так мне надо эту сделать лучше.
xgb.set.config(verbosity = 0) #don't print training update messages

train = TRAINING[[1]][[1]][[1]][[1]]
test = TEST[[1]][[1]]
best = list(eta = 0,
            gamma = 0,
            max_depth = 0,
...
for (eta in c(0.1, 0.5, 1)){
  for (gamma in c(0, 0.1, 1, 5, 10)){
    for (max_depth in c(6, 8, 10, 12)){
      for (scale_pos_weight in c(1, 2, 3, 5)){
        for (lambda in c(1, 1.1, 2, 5)){
          for (alpha in c(0, 0.1, 5, 1)){
        ...
        classifier = xgboost(data = as.matrix(train[-41]), #elimina la que buscamos
                     label = as.numeric(as.character(train[,41])), #consigue la última variable
                     nrounds = 13,
                     params = parameters
                     )

        # Predicting the Test set results
        y_pred = predict(classifier, newdata = as.matrix(test[-41]))
        y_pred = 1*(y_pred >= 0.5)
        #table(y_pred)
        #table(test[,41])
        # Making the Confusion Matrix
        cm = table(test[, 41], y_pred)
        if (ncol(cm) == 1){ #Assumed all test are majority class
          accuracy = 0
        }else{
          accuracy = (cm[1,1] + cm[2,2]) / (cm[1,1] + cm[2,2] + cm[1,2] + cm[2,1])
        } #Baseline accuracy is 0.775 for stunting
        if (accuracy > best$accuracy){
          best$eta = eta
          best$gamma = gamma
          best$max_depth = max_depth
          best$SPW = scale_pos_weight
          best$lambda = lambda
          best$alpha = alpha
          best$accuracy = accuracy
        }
...
best
# parameters = list(eta = 0.2, #default 0.3
#                   gamma = 0.1, #default 0
#                   max_depth = 6, #default 6
#                   subsample = 0.5,
#                   colsample_bytree = 0.5,
#                   scale_pos_weight = 1, #default 0; increase for unbalanced class distributions
...
# cm = table(test[, 41], y_pred)
# accuracy = (cm[1,1] + cm[2,2]) / (cm[1,1] + cm[2,2] + cm[1,2] + cm[2,1])
# accuracy #Baseline accuracy is 0.775 for stunting
```
