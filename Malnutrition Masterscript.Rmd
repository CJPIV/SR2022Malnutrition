...
# Rural has too few samples to narrow selection of rules down to ~10 (smallest possible is 23,260)
...
#Pretty quick.
#Identical to the above block of code, but running ARL on both Thinness datasets instead of both Stunting ones.

library(arules)
library(arulesViz)
################################################################################
data = data.frame(lapply(thinness, as.logical))
#Male vs. Female associations on Thinness
...
urban = subset(urban, Suburban == FALSE) #I don't know why this requires 2 lines; I had originally coded it as one line, but it would just copy everything from "data" into "urban". Coding is magic.
urban = urban[,-c(14,15)]
rules = apriori(urban, 
                parameter = list(support = 0.028,
                                 confidence = 0.05,
                                 maxlen = 10,
                                 target = "rules"),
                appearance = list(default = "lhs", rhs = c("Thinness")
...
inspect(rules[1:10])
#Why is this performing so poorly? Thinness => Urban's rules suck, but it has tons of data.
################################################################################
