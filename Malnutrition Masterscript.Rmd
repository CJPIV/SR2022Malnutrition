...
library(MASS)
          ...
          FSResults[[i]][[j]][[d]][[1]] = FeatureList[,c(1,4)]
################################################################################          
#MLoR
...
################################################################################
#InfoGain. This one is pretty quick.
library(FSelectorRcpp)
          
          infogain = information_gain(x = data[,-41],
                                      y = data[,41],
                                      type = 'infogain',
                                      equal = TRUE
                                      )
          infogain = infogain[order(infogain$importance, decreasing = TRUE),] #Sort
          for (index in 1:NUM_FEATURES){ #Look at top n most important features
            Feature = infogain[[index, 1]] #Get the name of the next most important feature
            if (infogain[[index, 2]] > 0){ #if significance > 0
              FeatureInd = which(names(Stunting) == Feature)
              FeatureList[[FeatureInd, 5]] = FeatureList[[FeatureInd, 5]] + 1 #Increase its value by 1
            }
          }
          FSResults[[i]][[j]][[d]][[4]] = FeatureList[,c(1,5)] #Update FeatureList
################################################################################
#mRMR. Pretty quick.
library(mRMRe)
