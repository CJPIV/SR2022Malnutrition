  ...
  for (j in 1:7){ #7 feature selection (FS) sets
    for (k in 1:10){ #10 test sets total
      for (l in 1:10){ #10 train/validation sets per test set
        ...
        svmGrid = expand.grid(C = c(0.1, 0.5, 1, 2, 3)
                              )
        SVM = train(x = train[-ncol(train)],
                           y = train[,ncol(train)],
                           method = 'svmLinear', #svmRadialWeights
                           metric = 'Accuracy',
                           tuneGrid = svmGrid,
                           trControl = control
                           )
        ...
        #y_pred = 1*(y_pred >= 0.5)
        y_pred = factor(colnames(y_pred)[max.col(y_pred)])
        CM = confusionMatrix(data = y_pred,
                             reference = as.factor(valid[,ncol(valid)])
                             )
        accuracy = CM$overall[["Accuracy"]]
...
for (j in 1:10){
  ...
  factors = append(factors, "Stunting")
...
for (j in 1:10){
  ...
  factors = append(factors, "Thinness")
...
for (j in 1:10){
  ...
  factors = append(factors, "Wasting")
...
for (i in 1:3){ #3 response sets (Stunting, Thinness, Wasting)
  ...
  for (j in 1:7){ #7 feature selection (FS) sets
    for (k in 1:10){ #10 test sets total
        train = TRAINVALID[[i]][[j]][[k]][[1]] #Import & coerce to matrix in order to scale
        if (length(ncol(train)) == 0){
          next
        }else if (ncol(train) == 0){
          next
        }
        train[,ncol(train)] = as.factor(train[,ncol(train)]) #Set response as factor
        
        test = TRAINVALID[[i]][[j]][[k]][[2]] #Prepare test; 1 test set for 10 train/valid sets (seen below)
        if (nrow(table(test[,ncol(test)])) != 2){
          next
        }
        test[,ncol(test)] = as.factor(test[,ncol(test)])
...
for (score in c("Accuracy", "F1")){
  for (response in c("Stunting", "Thinness", "Wasting")){
    RF = c()
    j = 1
    for (i in c(1,3,2,7,4,6,5)){ #Re-arrange results to the order we want them to appear in the heat map
