...
library(praznik)
          ...
          FSResults[[i]][[j]][[6]] = FeatureList[,c(1,7)]
################################################################################
        }
      }
  }
...
for (i in 1:3){
  for (j in 1:7){
    ...
    for (k in 1:6){
      ...
      T10Features[[i]][[j]][[k]] = T10Features[[i]][[j]][[k]][Selected,1]
    }
  }
}
```
...
DATAZ = list("Stunting" = list("Full" = list(),
                              "Urban" = list(),
                              "Nonurban" = list(),
                              "Male" = list(),
                              "Female" = list(),
...
for (i in 1:3){
  for (j in 1:7){
    ...
    data = DATAZ[[i]][[j]]
    # xVals = as.matrix(data[,-41])
    # yVals = as.vector(data[,41])
    # exploreModel = cv.glmnet(x = xVals,
    #                          y = yVals,
    #                          alpha = 1
    #                          )
    # lambda = exploreModel$lambda.min
    # bestModel = glmnet(x = xVals,
    #                    y = yVals,
    #                    alpha = 1,
    #                    lambda = lambda
    #                    )
    # LinResults[[i]][[j]] = bestModel
    
    linear = lm(paste0(names(data)[41],"~."),
                data = data
                )
    stepwise = stepAIC(linear, direction = "both", trace = FALSE)
    stepwise = tidy(stepwise, conf.level = 0.95)
    #linear = tidy(linear, conf.level = 0.95)
    stepwise = cbind(stepwise,
                     p.adjust(stepwise$p.value, method = "BH")
                     )
    stepwise = stepwise[-1,]
    significants = integer()
    for (k in 1:nrow(stepwise)){
...
```{r Odds Ratios for Requested Subsets}
...
for (i in c(1)){
  for (j in c(2,4,6)){
    data = DATA[[i]][[j]]
    logistic = glm(paste0(names(data)[41],"~FamilySize"),
                   data = data,
                   family = "binomial"
                   )
    factor = cbind(tidy(logistic, exponentiate = TRUE, conf.level = 0.95),
                   p.adjust(summary(logistic)$coefficients[2,4],method = "BH"),
                   exp(confint(logistic))
                   )
    results[index,] = factor[2,]
    results[index,1] = paste0(names(DATA)[[i]], names(DATA[[i]])[[j]], "FamilySize")
    index = index + 1
  }
}
...
#NOTE!!
#Go back into the DATA creation (line ~760) and re-run it with "subset" = TRUE. This will remove troublesome redundant categories within two of the subsets that result in this logistic regression failing.
#library(devtools)
library(broom)
library(car)
library(performance)
...
# Invert to look for protective factors
# for (i in 1:1032){
#   if(Stunting[[i,41]] == 1){
#     Stunting[[i,41]] = 0
#   }else{
...
for (setnum in 1:9){
  ...
  # datatable = table(factor(data[,1], levels = c("1", "0")),
  #                   factor(data[,2], levels = c("1", "0"))
  #                   )
  test = oddsratio(datatable, correction = FALSE)
  adj = oddsratio(datatable, correction = TRUE)
  test$p.value = test$p.value[[2,3]]
  test$measure = test$measure[2,]
  test$adj.PV = adj$p.value[[2,3]]
  RiskResults[[responseset]][[length(RiskResults[[responseset]])+1]] = test
