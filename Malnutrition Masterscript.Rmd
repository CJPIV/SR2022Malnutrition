...
```{r Handling "Na"s & Datatype Conversions, warning = FALSE}
...
################################################################################
#Turn categorical nominal "Address" column into binary rural/urban one; convert any addresses from within the city into 0s, else 1s
city = c("dilfire", "tesfatewahido", "setto-Yido", "amanuel", "jiren", "kito", "hermata mentina")
rural = c()
for (i in 1:1036){
  for (suburb in city){
    if (is.na(reducedSurvey[[i, 47]])){
      break
    }else if(tolower(reducedSurvey[[i, 47]]) == suburb){
      rural[[i]] = 0
      break
    }else{
      rural[[i]] = 1
...
for (sample in 1:1036){
  if (reducedSurvey[[sample, 10]] == 92.00){ #A sample's height was misinput as 92.00 meters.
...
for (sample in 1:1036){
  for (factor in c(30, 54, 63)){ #These columns were input as "NA/0" instead of "0/1"
    if (is.na(reducedSurvey[[sample, factor]])){
      reducedSurvey[[sample, factor]] = 0
...
#30, 54, 63 need to be increased by 1 I think
```
...
oneHotEncoded = dummy_cols(.data = imputed,
                     ...
                     split = ",")
oneHotEncoded = subset(oneHotEncoded, select = AL:HayFever1Yr)
#REPLACE [[48]] with 
#Iterate through and reduce some of the weird binaries (0/2 || 0/7 || 0/6) to 0/1


#Convert categorical columns into factors
#catIndices = c()
#imputed[,catIndices] = lapply(imputed[,catIndices], factor)
```
...
with(
  SURVEY,
  anthroplus_zscores(sex = imputed["Sex"]+1,
                              age_in_months = imputed["Age"]*12,
                              weight_in_kg = imputed["weight"],
                              height_in_cm = imputed["Height"]
                              )
)
# names(SURVEY)[] = "WAZ"
# names(SURVEY)[] = "HAZ"
