...
for (i in 1:10){
  for (j in 1:10){
    VALID[[3]][["MVLoR"]][[i]][[j]] = wasting[VALID[[3]][["MVLoR"]][[i]][[j]],][,c(1,41)]
  }
}
VALID[[3]][["IG"]] = VALID[[3]][["MVLoR"]]
VALID[[3]][["MRMR"]] = VALID[[3]][["MVLoR"]]
VALID[[3]][["JMI"]] = validSetsWasting #None
for (i in 1:10){
  for (j in 1:10){
    VALID[[3]][["JMI"]][[i]][[j]] = wasting[VALID[[3]][["JMI"]][[i]][[j]],][,c(1,12,41)]
  }
}
```
...
for (i in 1:10){
    TEST[[3]][["MVLoR"]][[i]] = TEST[[3]][["MVLoR"]][[i]][,c(1,41)]
}
TEST[[3]][["IG"]] = TEST[[3]][["MVLoR"]]
TEST[[3]][["MRMR"]] = TEST[[3]][["MVLoR"]]
TEST[[3]][["JMI"]] = testSetsWasting #None
for (i in 1:10){
    TEST[[3]][["JMI"]][[i]] = TEST[[3]][["JMI"]][[i]][,c(1,12,41)]
}
```
  ...
  for (j in 1:5){ #5 feature selection (FS) sets
    ...
    for (k in 1:10){ #10 test sets total
      ...
      for (l in 1:10){ #10 train/validation sets per test set
        ...
        if (ncol(cm) == 1){ #Assumed all test are majority class
          accuracy = 0
          F1 = 0
        }else{
...
```{r Extreme Gradient Boosting, results = FALSE}
set.seed(13)
library(xgboost)
library(caret)
library(tidyverse)
...
for (eta in c(0.5)){
  for (gamma in c(0.1)){
    for (max_depth in c(12)){
      for (scale_pos_weight in c(2)){
        for (lambda in c(1.1)){
          for (alpha in c(0)){
        ...
        y_pred = predict(classifier, newdata = as.matrix(test[-ncol(test)]))
        y_pred = 1*(y_pred >= 0.5)
        #table(y_pred)
        #table(test[,41])
        # Making the Confusion Matrix
...
XGB = function(training_set, test_set, x){
  training = training_set[-x, ]
  test = test_set[x, ]
  factor = names(training)[x]
  
  classifier = xgboost(data = as.matrix(training[-x]), #Matrix of independent vars.
                       label = training[,x], #Location of dependent var
                       nrounds = 15,
                       gamma = 0,
                       eta = 1
                       
                       ) 
  y_pred = predict(classifier, newdata = as.matrix(test[-x])) #Predict by using classifier on newdata's independent vars.
  y_pred = 1*(y_pred >= 0.5) #Binarize. The logic equation turns continuous results into TRUE/FALSE, and the "1*" converts it to 1/0
  cm = table(test[,x], y_pred)
  accuracy = (cm[1,1] + cm[2,2]) / (cm[1,1] + cm[2,2] + cm[1,2] + cm[2,1])
  return(accuracy)
}
