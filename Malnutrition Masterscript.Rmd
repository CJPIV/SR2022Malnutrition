...
VALID[[1]][["IG"]] = validSetsStunting
for (i in 1:10){
  for (j in 1:10){
    VALID[[1]][["IG"]][[i]][[j]] = stunting[VALID[[1]][["IG"]][[i]][[j]],][,c(13,29,5,24,8,38,23,28,41)]
  }
}
...
TEST[[1]][["IG"]] = testSetsStunting #IG
for (i in 1:10){
    TEST[[1]][["IG"]][[i]] = TEST[[1]][["IG"]][[i]][,c(13,29,5,24,8,38,23,28,41)]
}
...
for (i in 1){ #3 response sets (Stunting, Thinness, Wasting)
  for (j in 1){ #7 feature selection (FS) sets
    if (is.na(TRAINING[[i]][[j]])){
      next
    }
    for (k in 1:10){ #10 test sets total
...
for (score in c("Accuracy", "F1")){
  for (response in c("Stunting", "Thinness", "Wasting")){
    SVM = c()
    for (i in 1:7){
      SVM[i] = round(Scores_SVM[[response]][[i]][[score]], 3) #Round to 3 decimal places
    }
    XGB = c()
    for (i in 1:7){
      XGB[i] = round(Scores_XGB[[response]][[i]][[score]], 3) #Round to 3 decimal places
    }
    LogReg = c()
    for (i in 1:7){
      LogReg[i] = round(Scores_MVLoR[[response]][[i]][[score]], 3) #Round to 3 decimal places
    }
    mapdata = data.frame(Classifier = c(rep("RF", 7),
                                         rep("SVM", 7),
                                         rep("XGB", 7),
                                         rep("MVLoR", 7)
                                         ),
                         FS = c(rep(c("None","MVLoR","UVLoR","IG","MRMR","JMI","ChiSq"), 4)),
                         values = c(rep(0, 7),
                                    SVM,
                                    XGB,
                                    LogReg
    ...
    heatmap = ggplot(mapdata, aes(FS, Classifier, fill = values)) + geom_tile(color = "white", size = 0.5)
    heatmap = heatmap + geom_text(aes(label = values))
    #heatmap =  heatmap + geom_text(aes(label = values), col = 'black') + scale_fill_gradient(low = 'darkmagenta', high = 'gold')
    heatmap =  heatmap + geom_text(aes(label = values), col = 'black') +
      scale_fill_gradient2(low = "red",#rgb(163, 29, 42, maxColorValue = 255),
                           mid = "yellow",#rgb(254, 249, 192, maxColorValue = 255),
                           high = "green",#rgb(2,105,56, maxColorValue = 255),
                           midpoint = 0.5
                           )
    heatmap = heatmap + ggtitle(paste0(score," (%) on ", response))  #from Confusion Matrix Using Grid Search"))
