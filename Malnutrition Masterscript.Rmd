...
for (i in 1:3){ #3 response sets (Stunting, Thinness, Wasting)
  #if (i > 1){break}
  for (j in 1:7){ #5 feature selection (FS) sets
    ...
    for (k in 1:10){ #10 test sets total
      for (l in 1:10){ #10 train/validation sets per test set
        ...
        if (ncol(cm) < 2 | nrow(cm) < 2){ #Assumed all test are majority class
          accuracy = 0
          F1 = 0
        }else{
  ...
  for (j in 1:7){ #5 feature selection (FS) sets
    ...
    for (k in 1:10){ #10 test sets total
      for (l in 1:10){ #10 train/validation sets per test set
        ...
        y_pred = predict(RF, newdata = valid[-ncol(valid)])
        
        # Making the Confusion Matrix
        cm = table(valid[, ncol(valid)], y_pred)
        if (ncol(cm) < 2 | nrow(cm) < 2){ #Assumed all test are majority class
          accuracy = 0
          F1 = 0
        }else{
...
for (i in 1:3){ #3 response sets (Stunting, Thinness, Wasting)
  ...
  for (j in 1:7){ #5 feature selection (FS) sets
    ...
    for (k in 1:10){ #10 test sets total
      ...
      #test = as.matrix(test)
      #test[-ncol(test)] = as.data.frame(scale(test[-ncol(test)]))
      test[,ncol(test)] = as.factor(test[,ncol(test)])
      
      for (l in 1:10){ #10 train/validation sets per test set
        train = TRAINING[[i]][[j]][[k]][[l]] #Import & coerce to matrix in order to scale
        #train = as.matrix(train)
        #train[-ncol(train)] = as.data.frame(scale(train[-ncol(train)])) #Scale, coerce to DF
        train[,ncol(train)] = as.factor(train[,ncol(train)]) #Set response as factor

        # classifier = svm(x = train[-ncol(train)],
        #                  y = train[,ncol(train)],
        #                  #data = train,
        ...
        y_pred = predict(SVM, newdata = test[-ncol(test)])
        
        # Making the Confusion Matrix
        cm = table(test[, ncol(test)], y_pred)
        if (ncol(cm) != 2 | nrow(cm) != 2){ #Must be 2x2 CM, otherwise unusable
          accuracy = 0
          F1 = 0
        }else{
          accuracy = (cm[1,1] + cm[2,2]) / (cm[1,1] + cm[2,2] + cm[1,2] + cm[2,1])
  ...
  for (j in 1:7){ #5 feature selection (FS) sets
    ...
    for (k in 1:10){ #10 test sets total
      ...
      for (l in 1:10){ #10 train/validation sets per test set
        ...
        # classifier = xgboost(data = as.matrix(train[-ncol(train)]), #elimina la que buscamos
        #              label = as.numeric(as.character(train[,ncol(train)])), #consigue la última variable
        #              nrounds = 13,
        #              params = parameters
        #              )
        #cat(i)
...
mapdata = data.frame(FS = c(rep("None", 4),
                            rep("MVLoR", 4),
                            rep("UVLoR", 4),
                            rep("ChiSq", 4),
                            rep("MRMR", 4),
                     ...
                     classifiers = c(rep(c("RF", "SVM", "XGB", "MVLoR"), 7))
                     )
for (i in 1:3){ #For each response set
  for (j in 1:4){ #For each classifier
    for (k in 1:7){ #For each FS method
      
    }
  }
}

heatmap = ggplot(mapdata, aes(FS, classifiers))
heatmap =  heatmap + geom_text(aes(label = values), col = 'black') + scale_fill_gradient(low = 'yellow', high = 'red')
heatmap = heatmap + ggtitle("Accuracy (%) on Stunting Test Set from Confusion Matrix Using Grid Search")
heatmap + theme(text = element_text(size = 15))
heatmap
################################################################################
### Accuracy ###
'
  - Values are correlated across columns
      - For example: 68.81 refers to SVM_SMOTE/IG, 74.51 refers to RF_SMOTE, RelF
'

data = data.frame(Feature_Selection = c("IG", "IG", "IG", "IG",
                                        "RelF", "RelF", "RelF", "RelF",
                                        "LinReg", "LinReg", "LinReg", "LinReg",
                                        "Manual", "Manual", "Manual","Manual",
                                        "No FS", "No FS", "No FS", "No FS",
                                        "Comb FS", "Comb FS", "Comb FS", "Comb FS"), 
                  Classifiers = c("SVM_SMOTE", "RF_SMOTE", "SVM_Z", "RF_Z", 
                                  "SVM_SMOTE", "RF_SMOTE", "SVM_Z", "RF_Z",
                                  "SVM_SMOTE", "RF_SMOTE", "SVM_Z", "RF_Z",
                                  "SVM_SMOTE", "RF_SMOTE", "SVM_Z", "RF_Z",
                                  "SVM_SMOTE", "RF_SMOTE", "SVM_Z", "RF_Z",
                                  "SVM_SMOTE", "RF_SMOTE", "SVM_Z", "RF_Z"), 
                  values = c(68.81,	73.84,	61.39,	60.91,
                             68.24,	74.51,	61.39,	61.20,
                             67.37,	74.23,	59.07,	60.71,
                             67.37,	64.28,	61.29,	61.20,
                             65.53,	76.35,	56.86,	61.39,
                             69.30,	70.65,	61.39,	60.61))

ggp <- ggplot(data, aes(Feature_Selection, Classifiers)) + geom_tile(aes(fill = values)) # Creates default heatmap
ggp <- ggp + geom_text(aes(label = values))          # Add value labels to 
ggp <-  ggp + geom_text(aes(label = values), col = 'black') + scale_fill_gradient(low = 'yellow', high = 'red') # colors can be changed
ggp <- ggp + ggtitle("Accuracy (%) on Stunting Test Set from Confusion Matrix Using Grid Search")

ggp + theme(text = element_text(size = 15))     # Changes the font size of the axes labels and title              
