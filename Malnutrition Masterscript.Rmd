...
for (i in 1:10){
  for (j in 1:10){
    TRAINING[[1]][["MVLoR"]][[i]][[j]] = TRAINING[[1]][["MVLoR"]][[i]][[j]][,c(1,2,4,41)]
  }
}
...
for (i in 1:10){
  for (j in 1:10){
    TRAINING[[2]][["ChiSq"]][[i]][[j]] = TRAINING[[2]][["ChiSq"]][[i]][[j]]$data[,c(1,3,8,20,29,32,33,41)]
  }
}
...
TRAINING[[3]][["MRMR"]] = TRAINING[[3]][["IG"]]
TRAINING[[3]][["JMI"]] = SMOTEWasting #None
for (i in 1:10){
  for (j in 1:10){
    TRAINING[[3]][["JMI"]][[i]][[j]] = TRAINING[[3]][["JMI"]][[i]][[j]]$data[,c(1,5,12,41)]
  }
}
...
for (i in 1:10){
  for (j in 1:10){
    VALID[[1]][["MVLoR"]][[i]][[j]] = stunting[VALID[[1]][[2]][[i]][[j]],][,c(1,2,4,41)]
  }
}
...
for (i in 1:10){
  for (j in 1:10){
    VALID[[1]][["MRMR"]][[i]][[j]] = stunting[VALID[[1]][["MRMR"]][[i]][[j]],][,c(1,2,4,12,36,41)]
  }
}
VALID[[1]][["JMI"]] = validSetsStunting
for (i in 1:10){
  for (j in 1:10){
    VALID[[1]][["JMI"]][[i]][[j]] = stunting[VALID[[1]][["JMI"]][[i]][[j]],][,c(1:4,41)]
  }
}
VALID[[1]][["ChiSq"]] = NA
################################################################################
...
for (i in 1:10){
  for (j in 1:10){
    VALID[[2]][["IG"]][[i]][[j]] = thinness[VALID[[2]][["IG"]][[i]][[j]],][,c(5,8,24,41)]
  }
}
VALID[[2]][["MRMR"]] = NA
...
for (i in 1:10){
  for (j in 1:10){
    VALID[[2]][["JMI"]][[i]][[j]] = thinness[VALID[[2]][["JMI"]][[i]][[j]],][,c(5,8,23,41)]
  }
}
VALID[[2]][["ChiSq"]] = validSetsThinness
for (i in 1:10){
  for (j in 1:10){
    VALID[[2]][["ChiSq"]][[i]][[j]] = thinness[VALID[[2]][["ChiSq"]][[i]][[j]],][,c(1,3,8,20,29,32,33,41)]
  }
}
################################################################################
...
VALID[[3]][["MRMR"]] = VALID[[3]][["IG"]]
VALID[[3]][["JMI"]] = validSetsWasting #None
for (i in 1:10){
  for (j in 1:10){
    VALID[[3]][["JMI"]][[i]][[j]] = wasting[VALID[[3]][["JMI"]][[i]][[j]],][,c(1,5,12,41)]
  }
}
...
```{r Multivariate Logistic Regression Validation, warning = FALSE}
set.seed(13)
library(broom)
library(car)
library(performance)
  ...
  for (j in 1:7){ #7 feature selection (FS) sets
    ...
    for (k in 1:10){ #10 test sets total
      for (l in 1:10){ #10 train/validation sets per test set
        ...
        test = VALID[[i]][[j]][[k]][[l]] #Prepare test; 1 test set for 10 train/valid sets (seen below)
...
```{r Random Forest Validation, warning = FALSE}
set.seed(13)
...
```{r Support Vector Machine Validation, warning = FALSE}
set.seed(13)
library(e1071)
library(Matrix)
...
```{r Extreme Gradient Boosting Validation, results = FALSE, warning = FALSE}
set.seed(13)
library(xgboost)
library(caret)
library(tidyverse)
...
################################################################################
TRAINVALID[[1]][["None"]] = sansTestStunting
TRAINVALID[[1]][["MVLoR"]] = sansTestStunting
for (i in 1:10){
    TRAINVALID[[1]][["MVLoR"]][[i]] = TRAINVALID[[1]][["MVLoR"]][[i]][,c(1,2,4,41)]
}
TRAINVALID[[1]][["UVLoR"]] = sansTestStunting
for (i in 1:10){
    TRAINVALID[[1]][["UVLoR"]][[i]] = TRAINVALID[[1]][["UVLoR"]][[i]][,c(1,4,41)]
}
TRAINVALID[[1]][["IG"]] = NA #IG

TRAINVALID[[1]][["MRMR"]] = sansTestStunting
for (i in 1:10){
    TRAINVALID[[1]][["MRMR"]][[i]] = TRAINVALID[[1]][["MRMR"]][[i]][,c(1,2,4,12,36,41)]
}
TRAINVALID[[1]][["JMI"]] = sansTestStunting
for (i in 1:10){
    TRAINVALID[[1]][["JMI"]][[i]] = TRAINVALID[[1]][["JMI"]][[i]][,c(1:4,41)]
}
TRAINVALID[[1]][["ChiSq"]] = NA
################################################################################
TRAINVALID[[2]][["None"]] = sansTestThinness
for (i in 1:10){
    TRAINVALID[[2]][["None"]][[i]] = TRAINVALID[[2]][["None"]][[i]]
}
TRAINVALID[[2]][["MVLoR"]] = sansTestThinness
for (i in 1:10){
    TRAINVALID[[2]][["MVLoR"]][[i]] = TRAINVALID[[2]][["MVLoR"]][[i]][,c(11,17,41)]
}
TRAINVALID[[2]][["UVLoR"]] = sansTestThinness
for (i in 1:10){
    TRAINVALID[[2]][["UVLoR"]][[i]] = TRAINVALID[[2]][["UVLoR"]][[i]][,c(3,8,28,29,41)]
}
TRAINVALID[[2]][["IG"]] = sansTestThinness
for (i in 1:10){
    TRAINVALID[[2]][["IG"]][[i]] = TRAINVALID[[2]][["IG"]][[i]][,c(5,8,24,41)]
}
TRAINVALID[[2]][["MRMR"]] = NA
TRAINVALID[[2]][["JMI"]] = sansTestThinness
for (i in 1:10){
    TRAINVALID[[2]][["JMI"]][[i]] = TRAINVALID[[2]][["JMI"]][[i]][,c(5,8,23,41)]
}
TRAINVALID[[2]][["ChiSq"]] = sansTestThinness
for (i in 1:10){
    TRAINVALID[[2]][["ChiSq"]][[i]] = TRAINVALID[[2]][["ChiSq"]][[i]][,c(1,3,8,20,29,32,33,41)]
}
################################################################################
TRAINVALID[[3]][["None"]] = sansTestWasting
for (i in 1:10){
    TRAINVALID[[3]][["None"]][[i]] = TRAINVALID[[3]][["None"]][[i]]
}
TRAINVALID[[3]][["MVLoR"]] = sansTestWasting
for (i in 1:10){
    TRAINVALID[[3]][["MVLoR"]][[i]] = TRAINVALID[[3]][["MVLoR"]][[i]][,c(2,17,41)]
}
TRAINVALID[[3]][["UVLoR"]] = sansTestWasting
for (i in 1:10){
    TRAINVALID[[3]][["UVLoR"]][[i]] = TRAINVALID[[3]][["UVLoR"]][[i]][,c(1,16,41)]
}
TRAINVALID[[3]][["IG"]] = sansTestWasting
for (i in 1:10){
    TRAINVALID[[3]][["IG"]][[i]] = TRAINVALID[[3]][["IG"]][[i]][,c(1,41)]
}
TRAINVALID[[3]][["MRMR"]] = TRAINVALID[[3]][["IG"]]
TRAINVALID[[3]][["JMI"]] = sansTestWasting #None
for (i in 1:10){
    TRAINVALID[[3]][["JMI"]][[i]] = TRAINVALID[[3]][["JMI"]][[i]][,c(1,5,12,41)]
}
TRAINVALID[[3]][["ChiSq"]] = sansTestWasting #None
for (i in 1:10){
    TRAINVALID[[3]][["ChiSq"]][[i]] = TRAINVALID[[3]][["ChiSq"]][[i]][,c(1,16,20,41)]
}
```
...
for (i in 1:10){
    TEST[[1]][["MVLoR"]][[i]] = TEST[[1]][["MVLoR"]][[i]][,c(1,2,4,41)]
}
...
for (i in 1:10){
    TEST[[1]][["MRMR"]][[i]] = TEST[[1]][["MRMR"]][[i]][,c(1,2,4,12,36,41)]
}
TEST[[1]][["JMI"]] = testSetsStunting
for (i in 1:10){
    TEST[[1]][["JMI"]][[i]] = TEST[[1]][["JMI"]][[i]][,c(1:4,41)]
}
TEST[[1]][["ChiSq"]] = NA
################################################################################
...
for (i in 1:10){
    TEST[[2]][["IG"]][[i]] = TEST[[2]][["IG"]][[i]][,c(5,8,24,41)]
}
TEST[[2]][["MRMR"]] = NA
TEST[[2]][["JMI"]] = testSetsThinness
for (i in 1:10){
    TEST[[2]][["JMI"]][[i]] = TEST[[2]][["JMI"]][[i]][,c(5,8,23,41)]
}
TEST[[2]][["ChiSq"]] = testSetsThinness
for (i in 1:10){
    TEST[[2]][["ChiSq"]][[i]] = TEST[[2]][["ChiSq"]][[i]][,c(1,3,8,20,29,32,33,41)]
}
################################################################################
...
TEST[[3]][["MRMR"]] = TEST[[3]][["IG"]]
TEST[[3]][["JMI"]] = testSetsWasting #None
for (i in 1:10){
    TEST[[3]][["JMI"]][[i]] = TEST[[3]][["JMI"]][[i]][,c(1,5,12,41)]
}
...
```{r Multivariate Logistic Regression TEST, warning = FALSE}
set.seed(13)
library(broom)
library(car)
library(performance)
...
for (i in 1:3){ #3 response sets (Stunting, Thinness, Wasting)
  ...
  for (j in 1:7){ #7 feature selection (FS) sets
    ...
    for (k in 1:10){ #10 test sets total
        train = sansTestStunting[[i]][[j]][[k]] #Import & coerce to matrix in order to scale
        train[,ncol(train)] = as.factor(train[,ncol(train)]) #Set response as factor
        
        test = TEST[[i]][[j]][[k]][[l]] #Prepare test; 1 test set for 10 train/valid sets (seen below)
        if (length(range(test[,ncol(test)])) != 2){ #Should be both 0s and 1s as response vars
          next
        }else if (range(test[,ncol(test)])[2] != 1){
          next
...
```{r Random Forest TEST, warning = FALSE}
set.seed(13)
library(randomForest)
library(datasets)
library(caret)
...
```{r Support Vector Machine TEST, warning = FALSE}
set.seed(13)
library(e1071)
library(Matrix)
...
```{r Extreme Gradient Boosting TEST, results = FALSE, warning = FALSE}
set.seed(13)
library(xgboost)
library(caret)
library(tidyverse)
