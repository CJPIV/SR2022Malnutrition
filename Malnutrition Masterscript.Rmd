...
#Create varying sets of possible hyperparameters
#Run SVM with training+validation sets using each setting for hyperparameters
#   - But how do I actually do this part?
#   - How do I run SVM with a validation set?
#Test on test data
#Keep the set with the best results (accuracy/F1 scores)
...
for (i in 1:3){
  ...
  for (j in 1:5){ #each of 5 FS methods
    for (k in 1:10){ #x-coordinate of this training data
      for (l in 1:10){ #y-coordinate of this training data
        train = TRAINING[[i]][[j]][[k]][[l]]
        test = TEST[[i]][[j]][[k]][[l]]
      }
    }
...
library(e1071)
classifier = svm(formula = paste0(dep, "~."),
                 data = train,
...
# Я тоже хочу сделать что-то о тех, что мы расговаривали. Наш модель ещё не работает достаточно хорошо за публикировации, и так мне надо эту сделать лучше.

# Fitting XGBoost to the Training set
classifier = xgboost(data = as.matrix(train[-41]), #elimina la que buscamos
                     label = train[,41], #consigue la última variable
                     nrounds = 15,
                     gamma = 0, 
                     eta = 0.7
                     )
