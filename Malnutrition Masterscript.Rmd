...
#Char to booleans: 19, 21, 23, 31:42, 50:59, 64:68, 75 to "Tap water?", 75, 76, 82:95, 96 to numeric, 97:117
#Want to split up "Address" into rural/urban - how do manage that?
city = c("dilfire", "tesfatewahido", "setto-Yido", "amanuel", "jiren", "kito", "hermata mentina")
rural = c()
for (i in 1:1036){
  cat(reducedSurvey[[47,i]])
  cat("\n")
  for (suburb in city){
    cat(suburb)
    if (is.null(reducedSurvey[[47, i]])){
      break
    }
    else if(tolower(reducedSurvey[[47, i]]) == suburb){
      append(rural, 0) 
      break
    }else{
      append(rural, 1)
...
imputed = kNN(SURVEY, variable = colnames(SURVEY), k = 5, impNA = TRUE)
