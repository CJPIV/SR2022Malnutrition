...
write.csv(cbind(attr((MV_logi$terms), "term.labels"),
                summary(MV_logi)$coefficients[1:length(columns),4], #OOB
                exp(summary(MV_logi)$coef[1:length(columns)]),
                exp(confint(MV_logi))[1:length(columns), 1:2]),
...
df_uni = data.frame(Variable_Name=character(),
...
for (i in 1:length(columns_grouped)){
  ...
  for (j in 1:length(vec)){
    print(j)
    uni <- c(trm[j],
                 (exp(summary(uni_model)$coef[j,][1])),
                 exp((confint(uni_model)[j,])),
                 (summary(uni_model)$coef[j,][4])
                 )
    df_uni[nrow(df_uni) + 1,] = uni
  }
}

#This replaces NAs with 0 (in case the probabilities could not fit to 0 or 1)
df_uni[is.na(df_uni)] <- 0
#Adjusting the p-values
df_uni$P_Value_uni = p.adjust(df_uni$P_Value_uni)#, method = "BH")
#df_uni_Par$P_Value_uni = p.adjust(df_uni_Par$P_Value_uni)#, method = "BH")
#This writes the results to csv files.
write.csv(df_uni, file = paste("UVLoR_",responseVar,".csv", sep = ""), row.names = FALSE)
...
write.csv(df_counts, file = paste("UVLoR_",responseVar,".csv", sep = ""), 
          row.names = FALSE)
```
