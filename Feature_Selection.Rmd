...
sorted_relF_Stunting = relF[order(relF$importance), ] #returns a list of important features and their ranking

relF = relief(Thinness~., thinness, neighboursCount = 5)
sorted_relF_Thinness = relF[order(relF$importance), ]
```
...
sorted_Info_Stunting = infogain[order(infogain$importance), ] #returns a list of important features and their ranking

infogain = information_gain(Thinness~., data = thinness, type = 'infogain')
sorted_Info_Thinness = infogain[order(infogain$importance), ]
```
...
sortedMRMR = mRMR.classic(data = mrmr)
```
...
set.seed(13)  
train_indices = sample(1:nrow(stunting), 0.9*nrow(stunting))  # Create a vector of indices which is an 90% random sample
train = stunting[train_indices, ]     # Subset the Stunting data frame to training indices only
test = stunting[-train_indices, ]     # Exclude the training indices to create the test data set
#paste("train sample size: ", dim(train)[1])   # Training sample size
#paste("test sample size: ", dim(test)[1])     # Test sample size
model_logi_stunting = glm(Stunting~., data=stunting, family = "binomial")         # Fitting a binary logistic regression
#summary(model_logi)
tidy(model_logi_stunting, exponentiate = TRUE, conf.level = 0.95)

#Create train/test splits
set.seed(13)  
train_indices = sample(1:nrow(thinness), 0.9*nrow(thinness))  # Create a vector of indices which is an 90% random sample
train = thinness[train_indices, ]     # Subset the thinness data frame to training indices only
test = thinness[-train_indices, ]     # Exclude the training indices to create the test data set
#paste("train sample size: ", dim(train)[1])   # Training sample size
#paste("test sample size: ", dim(test)[1])     # Test sample size
model_logi_thinness = glm(Thinness~., data=thinness, family = "binomial")         # Fitting a binary logistic regression
#summary(model_logi)
tidy(model_logi_thinness, exponentiate = TRUE, conf.level = 0.95)
```
